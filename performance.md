...
5.  [Static Cache](#static-cache) (JS, CSS, Images with static caching services, NGINX `try_files`, etc.)
...
Static resources such as CSS, JavaScript, fonts, and images usually do not need to be generated by PHP or queried from the database on every request. The web server can serve these files directly from disk, often helped by the operating system's file cache, and browsers can keep local copies so repeat visits do not need to download the same files again.

The [`Cache-Control`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Cache-Control) response header controls how long browsers and intermediate caches can reuse a response. The older [`Expires`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Expires) header is still commonly sent for compatibility, but `Cache-Control` takes precedence when both are present. Long cache lifetimes are safest when static files are versioned. WordPress supports this for enqueued styles and scripts through the version argument, which appends a version query string to the file URL. When that version changes, browsers request the updated URL instead of reusing the old cached response.

Web servers can also decide whether a request needs PHP at all. On NGINX, this is commonly done with [`try_files`](https://nginx.org/en/docs/http/ngx_http_core_module.html#try_files), which serves a requested file directly when it exists on disk and only passes the request to PHP when it does not. Similar routing can also be used by file-based full page cache plugins, described in the [Full Page Cache](#full-page-cache) section, to serve cached HTML files without starting PHP.

As with the other caching layers, expiry matters. Static resources should use versioned file names or versioned URLs so updates reach visitors reliably, and any cached HTML pages must be purged when content changes.
